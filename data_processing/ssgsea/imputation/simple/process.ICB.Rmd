...
## 
```{r}
lee.sce =readRDS("~/liulab_home/data/single_cell/Lee_data/lee.sce.RDS")
...
library(Matrix)
library(magrittr)
rand.samp = sample(ncol(lee.cd3), 3000)
...
profvis({
big.prcomp = function(data, num.pcs, center=TRUE, scale=TRUE){
    data1 = scale(data, center=center, scale=scale)
    out = bigpca::big.PCA(data1,  pcs.to.keep = num.pcs)$PCs
    out
}
...
myone_level_clustering = function(subX, z_threshold, subsample= NULL) {
  ...
  pcs = tryCatch(expr = {
    ...
    # irlba::irlba(log(subX[features$gene, ] + 1), min(9, nrow(features) -1, ncol(subX) - 1))$v
    submat = mat = t(log(subX[features$gene, ] + 1))
    
    if(!is.null(subsample)){
        size.curr = max(100, nrow(mat) * subsample)
        if(size.curr< nrow(mat))
            submat = mat[sample(nrow(mat), size=size.curr),]  
    }
        eigenvec = big.prcomp(submat, min(9, nrow(features) -1, ncol(subX) - 1))
        pcs = mat %*% eigenvec
  }, error = function(e) {
    NA
  }, warning = function(w) {
