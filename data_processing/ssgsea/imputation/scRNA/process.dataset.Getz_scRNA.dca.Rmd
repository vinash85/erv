...
getwd()
icb.phenotype = fread("~/project/deeplearning/icb/data/Getz_scRNA/dca/val_prediction.csv")
...
indexes = grep(colnames(icb.phenotype), pattern = "embedding", value = T)
require(doMC)
require(foreach)
...
for(cell.type in cell.types) { 
    ...
    Pre.p[[cell.type]] = plot.aucs.hist(inx, indexes = indexes, filename = sprintf("/liulab/asahu/data/ssgsea/xiaoman/getz/dca/aucs/pretreatment_%s.pdf", cell.type), title= sprintf("Pretreatment %s", cell.type), aucs.dt = aucs.dt)
    
    inx = intersect(which(phenotype_sel.mod$assign.ident==cell.type), posttreatment.samples)
    aucs.dt = topaucs.genes[treat=="Post" & label==cell.type]
    Post.p[[cell.type]] = plot.aucs.hist(inx, indexes = indexes, filename = sprintf("/liulab/asahu/data/ssgsea/xiaoman/getz/dca/aucs/pretreatment_%s.pdf", cell.type), title= sprintf("Posttreatment %s  ", cell.type), aucs.dt = aucs.dt)
    
    
    inx =which(phenotype_sel.mod$assign.ident==cell.type)
    aucs.dt = topaucs.genes[treat=="All" & label==cell.type]
    All.p[[cell.type]] = plot.aucs.hist(inx, indexes = indexes, filename = sprintf("/liulab/asahu/data/ssgsea/xiaoman/getz/dca/aucs/pretreatment_%s.pdf", cell.type), title= sprintf("All %s  ", cell.type), aucs.dt = aucs.dt)
...
```{r}
pdf("/liulab/asahu/data/ssgsea/xiaoman/getz/dca/aucs/Pre.pdf", width=15, height=15)
...
tcga.phenotype = fread("~/project/deeplearning/icb/data/tcga/scrna.v1//val_prediction.csv")
tcga.dataset = fread("~/project/deeplearning/icb/data/tcga/scrna.v1/dataset.txt")
