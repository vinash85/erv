...
read.exp = function(expression.file, do.znorm=F) {
    aa = fread(expression.file)
    genes.curr= aa$Symbol
...
van.allen.exp = read.exp("/homes6/asahu/liulab_home/data/ssgsea/xiaoman/icb/my_expression//VanAllen2015_CTLA4_Melanoma_RNASeq.backup")

library(magrittr)
cbind(data.table(Symbol=colnames(van.allen.exp), t(van.allen.exp))) %>%
write.table(x = ., file="~/liulab_home/data/ssgsea/xiaoman/icb/expression/VanAllen2015_CTLA4_Melanoma_RNASeq", quote=F, sep="\t", row.names=F, col.names=T)
